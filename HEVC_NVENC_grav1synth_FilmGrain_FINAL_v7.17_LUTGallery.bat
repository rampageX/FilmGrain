...
rem  Windows drag-and-drop / multi-file production pipeline - FINAL v7.17 LUT Gallery
rem
rem  IMPORTANT:
...
rem Visual LUT gallery generated by LUT_Preview_Batch_v2.3_Gallery.bat
...
cls
echo ============================================================
echo              Film Look / LUT Gallery v7.17
echo ============================================================
echo.
echo LUT folder:
echo   "%LUT_ROOT%"
...
if not exist "%LUT_GALLERY_SELECTOR%" (
    ...
    echo.
    echo Keep this HEVC BAT beside the AV1 v7.17 BAT and the _LUT_Tools folder.
    echo.
    pause
    exit /b 0
)
...
rem RC 10 = user explicitly chose None; RC 11 = closed/cancelled.
if "%GALLERY_RC%"=="10" (
    del /q "%LUT_PICK%" >nul 2>&1
    exit /b 0
)
...
for /f "usebackq delims=" %%R in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$root=[IO.Path]::GetFullPath($env:LUT_ROOT).TrimEnd([char]92); $p=[IO.Path]::GetFullPath($env:LUT_PATH); $prefix=$root+[char]92; if($p.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)){$p.Substring($prefix.Length)}else{$p}"`) do set "LUT_RELATIVE=%%R"
...
rem Reject 1D/shaper CUBE files before preparing the HEVC filter path.
...
rem Convert that syntax to DOMAIN_MIN / DOMAIN_MAX without changing
rem the original LUT. The temporary file is kept for the HEVC job.
rem ------------------------------------------------------------
...
rem Validate before starting the actual HEVC encode.
