...
rem ============================================================
rem  AV1 NVENC + grav1synth Film Grain - FINAL v7.17 FilmLook-LUT
...
cls
echo ============================================================
echo        AV1 NVENC + grav1synth Film Grain FINAL v7.17 FilmLook-LUT
echo ============================================================
echo.
...
if not exist "%LUT_GALLERY_SELECTOR%" (
    echo.
    echo ERROR: LUT_Gallery_Selector.ps1 was not found beside this BAT.
...
rem Resolve a readable path relative to LUT_ROOT for the summary/output label.
set "LUT_RELATIVE=%LUT_NAME%"
for /f "usebackq delims=" %%R in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$root=[IO.Path]::GetFullPath($env:LUT_ROOT).TrimEnd('\\')+'\\'; $p=[IO.Path]::GetFullPath($env:LUT_PATH); if($p.StartsWith($root,[StringComparison]::OrdinalIgnoreCase)){$p.Substring($root.Length)}else{$p}"`) do set "LUT_RELATIVE=%%R"
...
rem Validate 3D CUBE structure. Resolve-specific LUT_3D_INPUT_RANGE is
rem converted later into the local filmlook.cube exactly as in v7.16.
