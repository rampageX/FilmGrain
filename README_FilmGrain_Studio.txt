...
- 编码方式：AV1 · grav1synth 胶片颗粒（默认）、HEVC · 扫描胶片颗粒、H.264 · x264 Grain（CPU / true 2-pass）；单个 AV1 还可选择不重编码添加/替换胶片颗粒。
- MP4（默认）与 MKV。
...
- AV1 / HEVC 均可选“同时生成 H.264 上传版”：上传副本固定使用 x264 Slow + tune grain + true 2-pass / High 8-bit，拥有独立自动/手动码率；OpenSVPFlow 60 fps 插帧开启时同样可用，且不会重复运行插帧。
