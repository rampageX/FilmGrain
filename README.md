...
v4.8.0 起 GUI 使用独立语言文件：

- `Lang\\zh-CN.ini`：简体中文，亦作为缺失翻译的 fallback。
- `Lang\\en-US.ini`：English。
- `Lang\\FilmGrain_Language.ini`：保存当前选择的语言代码。
- `Utils\\FilmGrain_Language.ps1`：只负责 UTF-8 语言文件读取、fallback、语言枚举与语言偏好保存。

语言切换仅改变 GUI 显示文本，不改变 StudioBridge、BAT/CLI 参数、编码器内部 ID、配置值或输出命名。为保持现有中文布局不变，简体中文继续使用原来的约 1320px 客户区宽度；英文界面使用更宽的默认窗口，并允许各语言通过语言文件单独定义窗口宽度。
