...
* 新增临时目录位置配置，支持源视频目录、系统临时目录和自定义目录。
* 系统临时目录与自定义临时目录统一使用 `FilmGrain_Studio` 子目录保存运行时临时文件，避免直接污染所选目录。
* 新增输出目标目录配置，可继续输出到源视频所在目录，也可指定自定义输出目录；默认行为仍保持输出到源视频当前目录。
* 临时目录与输出目标目录设置统一保存到根目录 `FilmGrain_Config.ini`。
* GUI 与 CLI 共用相同的目录配置核心，AV1、HEVC、x264、H.264 上传版及 AV1 不重编码添加/替换颗粒流程统一遵循对应路径设置。

### 改进与修复

* 增加临时目录和输出目录的路径有效性、写入权限及可用空间检查。
* 批量任务的输出空间检查按当前任务预计总需求进行判断。
* 修复自定义目录以反斜杠 `\` 结尾时，PowerShell 路径验证可能错误失败的问题。
* 路径检查会先规范化用于验证的目录路径，不改变用户实际选择的目标目录。
